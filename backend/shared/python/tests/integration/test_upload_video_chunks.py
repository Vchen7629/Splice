...
def test_upload_chunks_happy_path(
    ...
    monkeypatch.setattr(
        "shared_storage.queries.settings.BASE_STORAGE_URL", seaweedfs_url
    )
    job_id = "test-job-upload"

    storage_urls = [upload_video(job_id, path) for path in chunk_files]
...
def test_upload_single_chunk(
    ...
    url = upload_video("single-job", str(chunk))

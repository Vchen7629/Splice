...
func TestErrorCases(t *testing.T) {
	t.Run("Returns wrapped error when job directory cannot be created", func(t *testing.T) {
		result, err := service.SaveUploadedVideo(strings.NewReader("data"), "\x00", "video.mp4")
	...
	t.Run("Returns wrapped error when destination file cannot be created", func(t *testing.T) {
		...
		result, err := service.SaveUploadedVideo(strings.NewReader("data"), dir, "video\x00.mp4")
	...
	t.Run("Returns wrapped error when reading from src fails", func(t *testing.T) {
		...
		result, err := service.SaveUploadedVideo(&errorReader{}, dir, "video.mp4")
...
func TestSuccessResult(t *testing.T) {
	t.Run("Returns non-empty JobID and StoragePath on success", func(t *testing.T) {
		...
		result, err := service.SaveUploadedVideo(strings.NewReader("data"), dir, "clip.mp4")
	...
	t.Run("Strips path traversal from filename via filepath.Base", func(t *testing.T) {
		...
		result, err := service.SaveUploadedVideo(strings.NewReader("data"), dir, "../../../etc/passwd")
...
func TestFileSystem(t *testing.T) {
	t.Run("File is saved at StoragePath with correct content", func(t *testing.T) {
		...
		result, err := service.SaveUploadedVideo(bytes.NewReader(content), dir, "test.mp4")
		require.NoError(t, err)
	...
	t.Run("StoragePath follows storageDir/jobs/jobID/filename structure", func(t *testing.T) {
		...
		result, err := service.SaveUploadedVideo(strings.NewReader("data"), dir, "video.mp4")
		require.NoError(t, err)
	...
	t.Run("Each call produces a unique JobID and path", func(t *testing.T) {
		...
		result1, err1 := service.SaveUploadedVideo(strings.NewReader("data1"), dir, "video.mp4")
		result2, err2 := service.SaveUploadedVideo(strings.NewReader("data2"), dir, "video.mp4")

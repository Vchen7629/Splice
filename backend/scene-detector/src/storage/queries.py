...
def fetch_video(storage_url: str) -> str:
    ...
    try:
        ...
        logger.error("could not connect to seaweedfs", storage_url=storage_url, err=str(e))
        raise
...
def upload_video_chunks(job_id: str, chunk_paths: list[str]) -> list[str]:
    ...
    for chunk_path in chunk_paths:
        if not os.path.exists(chunk_path):
            logger.error("chunk video file not found before upload", chunk_path=chunk_path, job_id=job_id)
        ...
        try:
            ...
            logger.error("could not connect to seaweedfs", url=url, job_id=job_id, err=str(e))
            raise
        except requests.HTTPError as e:
            logger.error(
    ...
    logger.debug("uploaded video chunks to seaweedfs", job_id=job_id, count=len(storage_urls))
    return storage_urls
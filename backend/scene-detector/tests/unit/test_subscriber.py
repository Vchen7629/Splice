...
def make_mock_msg(data: dict[Any]) -> AsyncMock:
    msg = AsyncMock()
...
async def test_acks_on_success() -> None:
    ...
    with patch("src.nats.subscriber.process_job", new_callable=AsyncMock):
...
@pytest.mark.asyncio
async def test_naks_on_process_job_failure() -> None:
    msg = make_mock_msg({"job_id": "1", "storage_path": "/fake/idk.mp4"})
    mock_sub = MagicMock()
    mock_sub.messages = async_iter([msg])
    mock_js = AsyncMock(spec=JetStreamContext)
...
async def test_calls_process_job_per_message() -> None:
    ...
    with patch("src.nats.subscriber.process_job", new_callable=AsyncMock) as mock_process:                                                                   
        await raw_videos(mock_js)

...
func TestNewLoggerDevModeEnablesDebugLevel(t *testing.T) {
	logger := newLogger(&Config{ProdMode: false})

	assert.True(t, logger.Enabled(context.Background(), slog.LevelDebug))
}

func TestNewLoggerProdModeDisablesDebugLevel(t *testing.T) {
...
func TestConsumeVideoChunkErrorReturnsError(t *testing.T) {
	js := &test.MockJS{JStreamNameErr: assert.AnError}
	nc := &test.MockDrainer{}
	quit := make(chan os.Signal, 1)

	err := runCombiner(js, nc, silentLogger(), t.TempDir(), quit)

	require.ErrorIs(t, err, assert.AnError)
	assert.False(t, nc.DrainCalled, "Drain should not be called if consumer setup fails")
}

func TestBlocksUntilQuitSignal(t *testing.T) {
	quit := make(chan os.Signal, 1)
	done := make(chan error, 1)

	go func() {
		done <- runCombiner(okJS(), &test.MockDrainer{}, silentLogger(), t.TempDir(), quit)
	}()
...
func TestStopsConsumerOnQuit(t *testing.T) {
	consumer := &test.MockConsumer{}
	js := &test.MockJS{JStream: &test.MockStream{Cons: consumer}}
	quit := make(chan os.Signal, 1)
	quit <- os.Interrupt

	require.NoError(t, runCombiner(js, &test.MockDrainer{}, silentLogger(), t.TempDir(), quit))

	require.NotNil(t, consumer.Ctx)
	assert.True(t, consumer.Ctx.Stopped)
}

func TestDrainsNATSOnQuit(t *testing.T) {
	nc := &test.MockDrainer{}
	quit := make(chan os.Signal, 1)
	quit <- os.Interrupt

	require.NoError(t, runCombiner(okJS(), nc, silentLogger(), t.TempDir(), quit))

	assert.True(t, nc.DrainCalled)
}

func TestDrainErrorIsReturned(t *testing.T) {
	nc := &test.MockDrainer{DrainErr: assert.AnError}
	quit := make(chan os.Signal, 1)
	quit <- os.Interrupt

	err := runCombiner(okJS(), nc, silentLogger(), t.TempDir(), quit)
...
func TestLoadConfigMissingEnvFileReturnsError(t *testing.T) {
...
func TestLoadConfigDefaultValues(t *testing.T) {
	writeEnvFile(t, "") // empty file — godotenv succeeds, no overrides
